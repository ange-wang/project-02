...
for (var j in attacks) {
  ...
  if (country_name == geo[i].properties.ADMIN.toLowerCase() && active_year_list.includes(attacks[j].year)) {
    ...
    if (attacks[j].fatal.toLowerCase() == "y"){
      obj.fatal = ++fatal;
    }
    else{
      obj.none_fatal = ++nofatal;
    }

  }
}
//add count in mygeojson
 ...
 // Creating a geoJSON layer with the retrieved data
  L.geoJson(myGeoJSON, {
    ...
    onEachFeature: function(feature, layer) {
      ...
      layer.bindPopup("<h5>" + feature.properties+ "</h5> <hr>" + "Total Number of Cases: "+feature.count + "</br>" + "Fatal: "+feature.fatal + "</br> None Fatal: " + feature.none_fatal + "</hr>");
